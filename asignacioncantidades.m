function [N,NN,D,c,cc,Dcrit,Tw,lambd,mu,err,iter]=asignacioncantidades(q,Delta,Deltaw,alpha,beta,l,L,R,P,DN)
...
x=5*ones(2*P+R,1);
bbeta=0.5;
JH=JHH(x,q,Delta,Deltaw,alpha,beta,l,L,R,P);


while (iter<itermax)&&(err>tol)
    H=HH(x,q,Delta,Deltaw,alpha,beta,l,L,R,P,DN);
    JH=JHH(x,q,Delta,Deltaw,alpha,beta,l,L,R,P);
%rank(JH);
...
if m>-0.1
HP(:,1)=HH(x+(bbeta^jj)*d,q,Delta,Deltaw,alpha,beta,l,L,R,P,DN);
